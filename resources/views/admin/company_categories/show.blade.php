                ...
                <div class="col-md-6">
                    <table class="table table-bordered table-striped">
                        <tr>
                            ...
                            <td field-key='name'>{{ $company_category->name }}</td>
                        </tr>
                        <tr>
                            <th>@lang('quickadmin.company-categories.fields.description')</th>
                            <td field-key='description'>{!! $company_category->description !!}</td>
                        </tr>
                        <tr>
                            <th>@lang('quickadmin.company-categories.fields.photo')</th>
                            <td field-key='photo'>@if($company_category->photo)<a href="{{ asset(env('UPLOAD_PATH').'/' . $company_category->photo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/' . $company_category->photo) }}"/></a>@endif</td>
                        </tr>
                    </table>
                </div>
            </div><!-- Nav tabs -->
            ...
            @foreach ($companies as $company)
                <tr data-entry-id="{{ $company->id }}">
                    <td field-key='company_logo'>@if($company->company_logo)<a href="{{ asset(env('UPLOAD_PATH').'/' . $company->company_logo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/' . $company->company_logo) }}"/></a>@endif</td>
                                ...
                                <td field-key='photos'>@if($company->photos)<a href="{{ asset(env('UPLOAD_PATH').'/' . $company->photos) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/' . $company->photos) }}"/></a>@endif</td>
...
{!! Form::open(array(
        ...
        @endif
            ...
            <a href="{{ route('admin.company_categories.index') }}" class="btn btn-default">@lang('quickadmin.qa_back_to_list')</a>
        </div>
    </div>
@stop
